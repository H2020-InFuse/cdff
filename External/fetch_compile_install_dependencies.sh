...
if [[ ! -n $(find $DIR/package/ -name 'opencv*') ]]; then
	...
	cmake -D CMAKE_BUILD_TYPE=RELEASE -D WITH_FFMPEG=OFF -D CMAKE_INSTALL_PREFIX=$INSTALL_DIR -D BUILD_DOCS=OFF -D BUILD_EXAMPLES=OFF -D BUILD_TESTS=OFF -D ENABLE_CXX11=ON -D ENABLE_FAST_MATH=ON $DIR/opencv
	make
	install_function
...
if [[ ! -n $(find $DIR/package/ -name 'vtk*') ]]; then
	...
	cd $BUILD_DIR/vtk 
	cmake -D CMAKE_BUILD_TYPE=RELEASE -D CMAKE_INSTALL_PREFIX=$INSTALL_DIR -D BUILD_TESTING=OFF $DIR/vtk
	make
	install_function
