...
ImageUndistortionRectification::ImageUndistortionRectification()
{
	...
	parametersHelper.AddParameter<CameraConfigurationMode, CameraConfigurationModeHelper>("GeneralParameters", "CameraConfigurationMode", parameters.cameraConfigurationMode, DEFAULT_PARAMETERS.cameraConfigurationMode);
...
void ImageUndistortionRectification::configure()
{
	...
	{
		// CameraMatrix, distortionCoefficients and RectificationMatrix
		// are converted in cv::Mat format
		ConvertParametersToCvMatrices();
...
void ImageUndistortionRectification::process()
{
	...
	FrameConstPtr tmp = matToFrame.Convert(filteredImage);
	Copy(*tmp, outImage);
	delete(tmp);
}

ImageUndistortionRectification::CameraConfigurationModeHelper::CameraConfigurationModeHelper(
	const std::string& parameterName, CameraConfigurationMode& boundVariable, const CameraConfigurationMode& defaultValue)
