...
namespace ImageFiltering
{
    ...
    */
    ...
    class BackgroundExtraction : public ImageFilteringInterface
    {
        ...
        private:
            struct BackgroundExtractionOptionsSet
            {
                int requiredMatches;    // How many samples in the background model should be matched by the current pixel for us to consider that it is a a background pixel
                int samplesPerPixel;    // How many past samples should be stored in the background model for each pixel
                int foregroundLabel;    // The value of foreground points in the segmentation mask
                int backgroundLabel;    // The value of background points in the segmentation mask
                int distanceThreshold;  // What is the maximum distance between pixel values that should be considered similar ?
                int subsamplingFactor;  // At what frequency should samples in the background model be updated ?
                                        // This covers both spatial and temporal diffusion where the probability that the current pixel value will be inserted into the background model is '1 / subsamplingFactor'.
            };
            ...
            */
            cv::Point2i getPointFromNeighbourhoodOf(const cv::Point2i &pt, const cv::Size &size) const;

            /// This is the matrix containing the background model for each pixel.
            cv::Mat _backgroundModel;

            Helpers::ParametersListHelper parametersHelper;
            BackgroundExtractionOptionsSet parameters;
            static const BackgroundExtractionOptionsSet DEFAULT_PARAMETERS;
