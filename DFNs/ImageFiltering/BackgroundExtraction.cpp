...
{
    const FrameSize& frame_size = inImage.datasize;
    ASSERT(frame_size.width > 0 && frame_size.height > 0,
        "BackgroudSubtraction: input image should not be empty");
    // Data-depth is currently not set by the frame conversion functions
    // ASSERT(inImage.data_depth == 1,
    //     "BackgroundSubtraction: Input image should have a depth of 1 byte");
    ASSERT(inImage.frame_mode == asn1Sccmode_grayscale,
        "BackgroundSubtraction: Input image should be single channel grayscale");
}
                ...
                if (distance < parameters.distanceThreshold) {
                    ...
                    if (matches >= parameters.requiredMatches) {
                        ...
                        if (rand_subsample(random_engine) == 1) {
                            ...
                            background_model.at<uint8_t>(row, col, sample_to_discard) = frame.at<uint8_t>(row, col);
                        }
                        ...
                        if (rand_subsample(random_engine) == 1) {
                            ...
                            const int32_t sample_to_discard = rand_sample(random_engine);
                            background_model.at<uint8_t>(dst.y, dst.x, sample_to_discard) = frame.at<uint8_t>(row, col);
                        }
