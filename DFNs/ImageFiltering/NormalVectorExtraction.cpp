...
namespace Validators {
    namespace Frame {
        ...
        void HasStatus(const FrameWrapper::Frame &frame, FrameWrapper::FrameStatus status) {
            ASSERT(frame.frame_status == status, "Validators/Status - Invalid frame status");
        }

        void HasDepthOf(const FrameWrapper::Frame &frame, unsigned int depth) {
            ASSERT(frame.data_depth == depth, "Validators/Number - Invalid data depth");
        }

        void HasPixelSizeOf(const FrameWrapper::Frame &frame, unsigned int pixel_size) {
            ASSERT(frame.pixel_size == pixel_size, "Validators/Number - Invalid pixel size");
        }
    }
}
...
namespace dfn_ci
{
    ...
    void NormalVectorExtraction::process()
    {
        ...
        const auto* inPixels = reinterpret_cast<Vec3f*>(inImage.image.arr);
        ...
        auto* normalPixels = reinterpret_cast<Vec3f*>(normals->image.arr);

        const size_t width = inImage.datasize.width;
        for (size_t row = 1; row < inImage.datasize.height - 1; ++row)
        {
            for (size_t col = 1; col < inImage.datasize.width - 1; ++col)
            {
    ...
    {
        Validators::Frame::NotEmpty(frame);
        Validators::Frame::HasDepthOf(frame, 32);
        Validators::Frame::HasPixelSizeOf(frame, (32 / 8) * 3);
        Validators::Frame::HasFormatIn(frame, {FrameWrapper::MODE_UNDEFINED});
        Validators::Frame::HasStatus(frame, FrameWrapper::STATUS_VALID);
    }
