...
# --track-origins=no also traces back to where an uninitialised variable is first defined, to make fixing the "[UNC] Conditional jump or move depends on uninitialised value(s)" errors easier to resolve
...
../../../../valgrind/install/bin/valgrind --leak-check=full --show-leak-kinds=all --track-origins=no --undef-value-errors=no --error-exitcode=1 --child-silent-after-fork=yes -q --xml=yes --xml-file=leak_check_result/error.xml --log-file=leak_check_result/valgrind.log --fullpath-after=InFuse/ ./cdff-unit-tests
