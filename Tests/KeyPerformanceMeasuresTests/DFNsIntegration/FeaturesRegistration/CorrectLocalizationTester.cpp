...
void CorrectLocalizationTester::ExecuteDfns()
	...
	const VisualPointFeatureVector3D& inputModelFeatureVector = extractor->featuresOutput();

	extractor->pointcloudInput(*inputSceneCloud);
	extractor->process();
	const VisualPointFeatureVector3D& inputSceneFeatureVector = extractor->featuresOutput();

	PointCloudPtr inputModelFeatureCloud = NewPointCloud();
	PointCloudPtr inputModelSceneCloud = NewPointCloud();
	beginTime = clock();
	ConvertFeaturesVectorToPointCloud(inputModelFeatureVector, *inputModelFeatureCloud);
	endTime = clock();
	processingTime += float(endTime - beginTime) / CLOCKS_PER_SEC;
	ConvertFeaturesVectorToPointCloud(inputSceneFeatureVector, *inputModelSceneCloud);

	dfn->sourceCloudInput(*inputModelFeatureCloud);
	dfn->sinkCloudInput(*inputModelSceneCloud);
	...
	endTime = clock();
	processingTime += float(endTime - beginTime) / CLOCKS_PER_SEC;
	PRINT_TO_LOG("Processing took (seconds, only model feature extraction DFN is included in this time):", processingTime);
...
void CorrectLocalizationTester::ConvertFeaturesVectorToPointCloud(const VisualPointFeatureVector3D& vector, PointCloud& cloud)
	{
	Initialize(cloud);
	for(int keypointIndex = 0; keypointIndex < GetNumberOfPoints(vector); keypointIndex++)
		{
		AddPoint(cloud, GetXCoordinate(vector, keypointIndex), GetYCoordinate(vector, keypointIndex), GetZCoordinate(vector, keypointIndex)); \
		}
	}
