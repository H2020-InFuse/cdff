...
TEST_CASE( "Call to process", "[process]" ) 
	...
	canny.imageInput(new Frame());
	canny.process();

	// ----- save outputs
	FrameConstPtr output = canny.EdgeMapOutput();
...
TEST_CASE( "Call to configure", "[configure]" )
	{
	CannyEdgeExtractor2D canny;
	canny.setConfigurationFile("../tests/ConfigurationFiles/DFNs/EdgeMap2D/CannyEdgeExtractor2D_Conf1.yaml");
	canny.configure();	
	}
