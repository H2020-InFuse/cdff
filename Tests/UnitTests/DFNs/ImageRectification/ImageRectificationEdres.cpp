 ...
 * @addtogroup DFNsTest
 *
 * Unit Test for the DFN ImageRectificationEdres.
 *
 *
 * @{
 */
 ...
 * --------------------------------------------------------------------------
 */
TEST_CASE( "Call to process (ImageRectificationEdres)", "[process]" )
{
    // Prepare input data
    cv::Mat inputImage = cv::imread("../tests/Data/Images/EdresImages/NavL.png", cv::IMREAD_GRAYSCALE);
    ...
    // Instantiate DFN
    CDFF::DFN::ImageRectification::ImageRectificationEdres* rectification = new CDFF::DFN::ImageRectification::ImageRectificationEdres();
...
//    REQUIRE( output.data.msgVersion == array3D_Version );
    ...
    REQUIRE( output.intrinsic.cameraModel == asn1Scccam_MAPS );
