...
void PerformanceTestInterface::Run()
	...
	while ( SetNextInputs() )
		...
		while ( PrepareConfigurationFile() )
			{
			numberOfTests++;
			dfpc->setConfigurationFile(temporaryConfigurationFilePath);
			dfpc->setup();
 ...
 * --------------------------------------------------------------------------
 */
const std::string PerformanceTestInterface::temporaryConfigurationFileName = "PerformanceTest_TemporaryDFPC.yaml";
...
bool PerformanceTestInterface::PrepareConfigurationFile()	
	...
	for(int parameterIndex = changingParametersList.size()-1; parameterIndex >= 0; parameterIndex--)
		...
		YAML::Node dfnNode = configuration[parameter.dfnIndex];
		YAML::Node group = dfnNode[parameter.groupIndex];
...
void PerformanceTestInterface::ReadConfiguration()
	...
	for(unsigned dfnIndex = 0; dfnIndex < configuration.size(); dfnIndex++)
		...
		for(unsigned groupIndex = 0; groupIndex < configuration.size(); groupIndex++)
			{
			YAML::Node group = dfnNode[groupIndex];
